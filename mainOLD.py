...
import io, base64, os, random, cv2
...
app = Flask(__name__)
app.config['REMEMBER_COOKIE_DURATION'] = timedelta(minutes=180)
app.secret_key = 'Keykey11!'
...
def check_session_expiry():
    if current_user.is_authenticated:
        if 'expiry' in session:
            ...
            expiry = datetime.utcnow() + app.config['REMEMBER_COOKIE_DURATION']
    ...
    if request.method == 'POST':
        ...
        try:
            if dbExists(tag):
                ...
                if not tag.isalpha():
                    return logResp(f"danger^2500^Tag contains non-alphabetical characters. '{tag}'",tag)
                response = make_response(dbAdd(tag,date))
...
def dbAdd(tag,date,model):
    try:
        ...
        try:
            print(date)
            p = Profile(tag=tag, school=metadata["schools"][from_hex(tag[3:4])], location="", room=from_hex(tag[4:6]), item=metadata["items"][from_hex(tag[6:])], comments="", lad="Never", dinst="Unset")
            ...
            print(e)
            return logResp(f"danger^1500^Python Exception! Tag might formated incorrectly.",tag)  
        db.session.add(p)
        db.session.commit()
...
def dbEdit():
    try:
        ...
        setattr(data,"lad",now.strftime('%m/%d/%Y %I:%M %p'))
        db.session.commit()
...
def logResp(alert,dispData):
    ...
    dateHead = datetime.now().strftime('%m/%d/%Y %I:%M %p')
...
def barcodeGenerator():
    if request.method == 'POST':
        ...
        pdf_data = base64.b64encode(sheet).decode('utf-8')
        return pdf_data
    else:
        metadata = getMetadata()
        ...
        return render_template('barcodeGenerator.html', schools=schools, ipOct=ipOct, items=items, locations=locations, ip = request.remote_addr )
...
def reportGenerator():
    if request.method == 'POST':
        data = request.get_json()
        ## TODO ##  ---  Do the function required to generate report.
        #pdf_data = base64.b64encode(sheet).decode('utf-8')
        #return pdf_data
    else:
        metadata = getMetadata()
...
def blankSheet():
    ...
    return Image.open("resources/avery.jpg")
...
def labelSheet(tags):
    ...
    for i, item in enumerate(tags):
        ...
        font = ImageFont.truetype('resources/FiraCode-Regular.ttf', 30)
...
def datavis(page):
    try:
        ...
        headers = ['Tag', 'School', 'Location', 'Room', 'Item']
        rows = [[i.tag, i.school, i.location, i.room, i.item] for i in assets[(24*(int(page)-1)):(24*(int(page)))]]
        if len(rows) == 0:
            if "/datavis" not in str(request.referrer) and str(request.referrer)!="None":
...
def itemTemplate(tag):
    ...
    return render_template('itemTemplate.html',tag=tag, school=data.school, location=data.location, room=data.room, item=data.item, model=data.model, comments=data.comments, lad=data.lad, dinst=data.dinst)
